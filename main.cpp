...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (nGoals == vGoals.size()) {
			DrawString(4, 4, "Done! " + std::to_string(nGoals) + " / " + std::to_string(vGoals.size()) , olc::WHITE);
			current_level = current_level + 1;
			LoadLevel(current_level);
		}
		else {
		
			DrawString(4, 4, " Level:" + std::to_string(current_level)+ " Goals: " + std::to_string(nGoals) + " / " + std::to_string(vGoals.size()) + " Moves:"+ std::to_string(c), olc::WHITE);
